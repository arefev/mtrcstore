...
server-run: server-build
	./cmd/server/server -d=${DATABASE_DSN} -k="${SECRET_KEY}" -a="localhost:8081" -crypto-key="./private.pem" -c="./server-config.json"
...
agent-run: agent-build
	./cmd/agent/agent -r 2 -p 1 -k="${SECRET_KEY}" -a="localhost:8081" -crypto-key="./public.pem"
